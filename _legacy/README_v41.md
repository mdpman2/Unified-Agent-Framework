...
> **⚡ v5.0 Runner-Centric Redesign 출시!** — 49개 모듈 → 9개, 16개 브릿지 → Top 3 엔진, `run_agent("질문")` 한 줄로 끝. **[v5 문서 보기 →](unified_agent_v5/README.md)**
...
[![Tests](https://img.shields.io/badge/Tests-28%2F28%20Scenarios%20Passed-success.svg)](#-테스트)
[![Coverage](https://img.shields.io/badge/Coverage-100%25-brightgreen.svg)](#-테스트)

> **v4.1.0** - 🆕 **2026년 2월 14일** | **v4.1 신규**: Agent Identity (Entra ID), Browser Automation & CUA, Deep Research, OpenTelemetry Observability, Middleware Pipeline, Agent Triggers | **v4.0 핵심**: Universal Agent Bridge, Session Tree, PDCA, Security Guardrails, A2A+MCP

## 📖 목차

- [v4.1 주요 업데이트](#-v41-주요-업데이트-2026년-2월) - Agent Identity, Browser Automation, Deep Research, Observability, Middleware, Triggers
...
## 🆕 v4.1 주요 업데이트 (2026년 2월)

> **6가지 최신 기술 통합** — Azure AI Foundry Agent Service, Microsoft Agent Framework, OpenAI CUA의 2026년 2월 최신 기능을 프레임워크에 반영

### 📊 v4.1 한눈에 보기

| 기능 | 설명 | 관련 기술 |
|------|------|-----------|
| 🔐 **Agent Identity** | Microsoft Entra ID 에이전트 전용 ID/RBAC | Azure Entra Agent Identity |
| 🌐 **Browser Automation & CUA** | Playwright 브라우저 자동화 + Computer Use Agent | Azure Foundry + OpenAI CUA |
| 🔬 **Deep Research** | 다단계 자율 연구 에이전트 (o3-deep-research) | Azure Deep Research Tool |
| 📊 **Observability** | OpenTelemetry 분산 추적/메트릭/로깅 | Azure Monitor + OTel |
| 🔧 **Middleware Pipeline** | 요청/응답 미들웨어 체인 (인증, 캐시, 필터링) | MS Agent Framework Pattern |
| ⚡ **Agent Triggers** | 이벤트/스케줄/웹훅 기반 에이전트 자동 호출 | Azure Logic Apps Pattern |

### 1. 🔐 Agent Identity — Microsoft Entra 에이전트 인증/인가

Microsoft Entra ID에서 에이전트 전용 ID를 프로비저닝하고, Azure RBAC로 최소 권한을 적용합니다.

```python
from unified_agent import (
    AgentIdentityProvider, AgentRBACManager,
    PermissionScope, ScopedPermission
)

# 에이전트 ID 생성 (최소 권한 원칙 적용)
...
can_read = rbac.check_permission(identity.agent_id, "storage:read")  # True

# 에이전트 간 위임 인증
delegation = await provider.delegate(
    from_agent=identity, to_agent_id="writer-agent",
    scopes=[PermissionScope.FILE_WRITE], ttl_minutes=30
)
```

> 💡 **아이디어**: "에이전트도 사람처럼 ID와 권한이 필요하다 — 최소 권한 원칙(Least Privilege)으로 안전하게"

### 2. 🌐 Browser Automation & CUA — 브라우저 자동화 + 컴퓨터 사용 에이전트

Playwright 기반 헤드리스 브라우저와 OpenAI Computer Use Agent(CUA)를 통합합니다.

```python
from unified_agent import (
    BrowserAutomation, ComputerUseAgent,
    BrowserConfig, CUAConfig, CUAEnvironment
)

# Playwright 브라우저 자동화 — 웹 스크래핑, 폼 입력, 스크린샷
browser = BrowserAutomation(BrowserConfig(
    headless=True, timeout=30.0, screenshot_on_error=True
))
session = await browser.create_session("https://azure.microsoft.com")
page_text = await session.get_text()
await session.screenshot("azure_portal.png")

# OpenAI CUA — AI가 컴퓨터를 직접 조작
cua = ComputerUseAgent(CUAConfig(
    model="computer-use-preview",
    environment=CUAEnvironment.BROWSER,
    safety_checks=True  # 안전 검사 필수!
))
result = await cua.execute("Azure Portal에서 리소스 그룹 목록을 확인해주세요")
print(result.actions)  # 클릭, 타이핑, 스크롤 등 자동 실행
```

> 💡 **아이디어**: "에이전트가 API뿐 아니라 화면도 다룰 수 있어야 완전한 자동화"

### 3. 🔬 Deep Research — 다단계 자율 연구 에이전트

Azure o3-deep-research 모델을 활용한 자율 연구: 기획 → 검색 → 수집 → 분석 → 종합 → 검증 → 보고서

```python
from unified_agent import (
    DeepResearchAgent, ResearchConfig,
    SynthesisFormat, SourceType
)
...
agent = DeepResearchAgent(ResearchConfig(
    ...
    max_sources=50,
    synthesis_format=SynthesisFormat.REPORT,
    enable_citation=True,
    enable_verification=True,
    allowed_source_types=[SourceType.WEB, SourceType.ACADEMIC, SourceType.API]
))

# 자율 연구 실행 (수 분 ~ 수십 분 소요)
result = await agent.research(
    topic="2026년 AI 에이전트 프레임워크 트렌드 분석",
    depth="comprehensive"
)

print(result.report)          # 종합 보고서
print(result.citations)       # 인용 목록 (출처 포함)
print(result.confidence)      # 신뢰도 점수
print(result.checkpoints)     # 중간 체크포인트
```

> 💡 **아이디어**: "검색 한 번으로 끝나지 않는 깊은 연구 — 인간 연구자처럼 계획하고, 수집하고, 검증한다"

### 4. 📊 Observability — OpenTelemetry 기반 에이전트 관찰성

분산 추적, 메트릭, 로깅을 OpenTelemetry 네이티브로 통합하여 에이전트를 완전히 모니터링합니다.

```python
from unified_agent import (
    ObservabilityPipeline, ObservabilityConfig,
    AgentTelemetry, AlertRule, AlertSeverity
)
...
# 도구 호출 트레이싱
with telemetry.trace_tool_call("web_search") as span:
    span.set_parameters({"query": "AI trends 2026"})
    results = await search(...)

# 알림 설정
pipeline.alert_manager.add_rule(AlertRule(
    name="high_latency",
    metric="llm.response_time_ms",
    threshold=5000.0,
    severity=AlertSeverity.WARNING
))
```

> 💡 **아이디어**: "보이지 않으면 관리할 수 없다 — 에이전트의 모든 행동을 추적하고 수치화"

### 5. 🔧 Middleware Pipeline — 요청/응답 미들웨어

ASP.NET Core / Microsoft Agent Framework 스타일의 미들웨어 체인으로 인증, 레이트 리밋, 캐싱, 컨텐츠 필터링을 파이프라인에 추가합니다.

```python
from unified_agent import (
    MiddlewareManager, MiddlewareConfig,
    LoggingMiddleware, AuthMiddleware,
    RateLimitMiddleware, RetryMiddleware,
    ContentFilterMiddleware, CacheMiddleware, MiddlewareContext
)
...
# 미들웨어 추가 (우선순위 자동 정렬: 인증 → 레이트리밋 → 필터 → 로깅 → 캐시)
manager.add(AuthMiddleware(provider="entra_id"))
manager.add(RateLimitMiddleware(max_rpm=60, max_tpm=100_000))
manager.add(ContentFilterMiddleware(enable_pii_detection=True))
...
# 요청 처리 (에이전트 실행 전후로 미들웨어 자동 적용)
context = MiddlewareContext(agent_id="research-agent", request=user_request)
result = await manager.process(context, agent_fn=my_agent.run)
```

> 💡 **아이디어**: "에이전트 로직과 인프라 관심사를 분리 — 인증, 보안, 캐싱은 미들웨어에 위임"

### 6. ⚡ Agent Triggers — 이벤트 기반 에이전트 자동 호출

Azure Logic Apps 패턴을 적용한 이벤트/스케줄/웹훅 기반 에이전트 트리거 시스템입니다.

```python
from unified_agent import (
    TriggerManager, TriggerConfig,
    EventTrigger, TriggerCondition
)
...
async def handle_document(event):
    """새 문서가 업로드되면 자동으로 분석 에이전트 실행"""
    agent = create_analysis_agent()
    return await agent.run(f"문서 분석: {event.data}")

# 스케줄 트리거: 매일 오전 9시 보고서 생성
@manager.on_schedule("0 9 * * *")
async def daily_report(event):
    agent = create_report_agent()
    return await agent.run("일일 보고서 생성")

# 웹훅 트리거: GitHub PR 이벤트 수신
...
# 이벤트 디스패치
from unified_agent import TriggerEvent
event = TriggerEvent(event_type="document.uploaded", data={"path": "/docs/report.pdf"})
results = await manager.dispatch_event(event)
```

> 💡 **아이디어**: "에이전트는 사람이 호출하는 것만이 아니다 — 이벤트가 에이전트를 깨운다"
...
v4.1에서 **6가지 최신 기술** (Agent Identity, Browser Automation, Deep Research, Observability, Middleware, Triggers)을 추가하여 총 **49개 모듈, 480+ API**의 완전한 모듈화 아키텍처로 확장되었습니다:

### 패키지 구조

```
unified_agent/
├── __init__.py          # 패키지 진입점 (480+ 공개 API export)
...
├── agent_identity.py      # v4.1 NEW! 에이전트 ID (AgentIdentity, AgentRBACManager, Entra ID)
├── browser_use.py         # v4.1 NEW! 브라우저 자동화 (BrowserAutomation, ComputerUseAgent, CUA)
├── deep_research.py       # v4.1 NEW! 딥 리서치 (DeepResearchAgent, SourceCollector, o3-deep-research)
├── observability.py       # v4.1 NEW! 관찰성 (ObservabilityPipeline, AgentTelemetry, OpenTelemetry)
├── middleware.py          # v4.1 NEW! 미들웨어 (MiddlewareManager, AuthMiddleware, CacheMiddleware)
└── agent_triggers.py      # v4.1 NEW! 에이전트 트리거 (TriggerManager, EventTrigger, ScheduleTrigger)
```
...
| `dataclass(slots=True)` | 49개 모듈 전체 | 메모리 사용량 20-30% 감소 |
...
| `from __future__ import annotations` | 49개 모듈 전체 | 타입 힌트 지연 평가, import 시간 단축 |
| 모던 typing (`X \| None`) | 49개 모듈 전체 | `Optional[X]` → `X \| None`, `Dict` → `dict` |
...
v4.1에서는 28개 시나리오 기반의 포괄적인 테스트 스위트를 제공합니다.

### 테스트 설계 원칙

- **`run_async()` 헬퍼** — 반복되는 `async def / asyncio.run()` 패턴을 한 줄로 대체
- **`_TESTS` 레지스트리** — 28개 테스트 함수를 리스트로 관리, `main()`에서 루프 실행
- **표준화된 에러 핸들링** — `TestRunner.record()`로 통일된 성공/실패 출력
- **미사용 import 제거** — 49개 불필요 import 정리 완료

### 테스트 실행

```bash
# 전체 시나리오 테스트 (28개 시나리오, 49개 모듈, 100% 커버리지)
# Windows에서는 인코딩 설정 필요: $env:PYTHONIOENCODING = "utf-8"
...
======================================================================
  ...
  실행 시간: 2026-02-14 01:10:37
  Python: 3.13.4
  테스트 시나리오: 28개 (Core ~ v4.1)
======================================================================
  [PASS] Core Import
  [PASS] Core Framework
  ...
  [PASS] Evaluation (PDCA)
  [PASS] Responses API
  [PASS] Video Generation
  [PASS] Image Generation
  [PASS] Open Weight Models
  [PASS] Universal Agent Bridge
  [PASS] Agent Identity
  [PASS] Browser Automation & CUA
  [PASS] Deep Research
  [PASS] Observability
  [PASS] Middleware Pipeline
  [PASS] Agent Triggers
----------------------------------------------------------------------
  총 테스트: 28개
  통과: 28개
...
| Agent Identity | v4.1 | AgentIdentity, AgentRBACManager, ScopedPermission, Delegation | ✅ |
| Browser Automation & CUA | v4.1 | BrowserAutomation, ComputerUseAgent, SafetyChecker | ✅ |
| Deep Research | v4.1 | DeepResearchAgent, SourceCollector, SynthesisEngine, CitationManager | ✅ |
| Observability | v4.1 | ObservabilityPipeline, AgentTelemetry, MetricsCollector, AlertManager | ✅ |
| Middleware Pipeline | v4.1 | MiddlewareManager, AuthMiddleware, RateLimitMiddleware, CacheMiddleware | ✅ |
| Agent Triggers | v4.1 | TriggerManager, EventTrigger, ScheduleTrigger, WebhookTrigger | ✅ |
...
│
├── 📦 unified_agent/              # 핵심 패키지 (49개 모듈, 480+ API)
│   ├── __init__.py               # 패키지 진입점 (480+ export)
...
│   ├── agent_identity.py         # [v4.1 NEW!] 에이전트 ID (Entra ID RBAC)
│   ├── browser_use.py            # [v4.1 NEW!] 브라우저 자동화 + CUA
│   ├── deep_research.py          # [v4.1 NEW!] 딥 리서치 (o3-deep-research)
│   ├── observability.py          # [v4.1 NEW!] OpenTelemetry 관찰성
│   ├── middleware.py             # [v4.1 NEW!] 미들웨어 파이프라인
│   └── agent_triggers.py         # [v4.1 NEW!] 이벤트 기반 트리거
│
...
│   └── korean-writer/
│
├── 🧪 test_v41_all_scenarios.py    # 통합 테스트 (28개 시나리오, 49개 모듈, 레지스트리 기반)
...
| 버전 | 날짜 | 주요 변경사항 |
|------|------|-------------|
| **4.1.0** | 2026-02-14 | 🆕 **6가지 최신 기술 통합**: Agent Identity (Entra ID RBAC), Browser Automation & CUA (Playwright + OpenAI CUA), Deep Research (o3-deep-research), Observability (OpenTelemetry), Middleware Pipeline, Agent Triggers (Event-Driven) — 49개 모듈, 480+ API. 테스트 코드 최적화 (`run_async` 헬퍼, 미사용 import 정리, 레지스트리 기반 main) |
...
| | | 🆕 **AI Agent Framework 에코시스템**: **OpenAI Agents SDK v0.8.1** (Handoff, Session, HITL, Voice), **Google ADK v1.24.1** (Workflow Agent, A2A), **CrewAI v1.9.3** (Crews+Flows), **A2A Protocol v0.3.0** (Linux Foundation), **MS Agent Framework Preview** (Graph Workflow, DevUI), **AG2 AgentOS**, **SK Agent Framework Orchestration** (16개 프레임워크, 49개 모듈, 480+ API) |
