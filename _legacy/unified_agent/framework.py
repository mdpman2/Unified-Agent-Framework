...
# opentelemetry, semantic_kernel — lazy import (method-level)
# 사용 시점에만 로드하여 패키지 초기 import 속도 향상
if TYPE_CHECKING:
    from opentelemetry import trace as _trace_mod
    from semantic_kernel import Kernel as _Kernel
    from semantic_kernel.connectors.ai.open_ai import AzureChatCompletion as _AzureChat
...
class UnifiedAgentFramework:
    ...
    def __init__(
        self,
        kernel: Any,  # semantic_kernel.Kernel
        ...
        if enable_telemetry:
            ...
            self.tracer = _otel_trace.get_tracer(__name__)
        else:
            self.tracer = None
    ...
    # ========================================================================
    # v4.1 팩토리 메서드 — 최신 기술 기능 편의 접근
    # ========================================================================

    def create_agent_identity_provider(self, tenant_id: str | None = None) -> Any:
        """
        v4.1 Agent Identity Provider 생성

        사용법:
            provider = framework.create_agent_identity_provider(tenant_id="your-tenant")
            identity = await provider.provision_agent(name="research-agent")
        """
        from .agent_identity import AgentIdentityProvider
        return AgentIdentityProvider(tenant_id=tenant_id)

    def create_browser_automation(self, config: Any = None) -> Any:
        """
        v4.1 Browser Automation 인스턴스 생성

        사용법:
            browser = framework.create_browser_automation()
            session = await browser.create_session("https://example.com")
        """
        from .browser_use import BrowserAutomation, BrowserConfig
        return BrowserAutomation(config or BrowserConfig())

    def create_deep_research_agent(self, config: Any = None) -> Any:
        """
        v4.1 Deep Research Agent 생성

        사용법:
            agent = framework.create_deep_research_agent()
            result = await agent.research("AI Agent 프레임워크 비교 분석")
        """
        from .deep_research import DeepResearchAgent, ResearchConfig
        return DeepResearchAgent(config or ResearchConfig())

    def create_observability_pipeline(self, config: Any = None) -> Any:
        """
        v4.1 Observability Pipeline 생성

        사용법:
            pipeline = framework.create_observability_pipeline()
            pipeline.start()
        """
        from .observability import ObservabilityPipeline, ObservabilityConfig
        return ObservabilityPipeline(config or ObservabilityConfig())

    def create_middleware_manager(self) -> Any:
        """
        v4.1 Middleware Manager 생성

        사용법:
            manager = framework.create_middleware_manager()
            manager.add(AuthMiddleware(api_key="..."))
        """
        from .middleware import MiddlewareManager
        return MiddlewareManager()

    def create_trigger_manager(self) -> Any:
        """
        v4.1 Trigger Manager 생성

        사용법:
            manager = framework.create_trigger_manager()
            manager.register(EventTrigger(event_type="user.message"))
        """
        from .agent_triggers import TriggerManager
